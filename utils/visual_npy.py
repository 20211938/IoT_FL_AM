...
def visualize_defect_types_samples(data_dir, client_identifier_dict, label_mapping, min_pixels=100):
    ...
    for defect_value in sorted_defect_types:
        ...
        print(f"\n결함 유형 {defect_value} (레이블 인덱스: {label_idx}):")
        print(f"  - 샘플 파일: {file_name}")
        print(f"  - 픽셀 수: {pixel_count} ({ratio:.2f}%)")
        print(f"  - 마스크 크기: {mask.shape}")


# 사용 예시 (노트북 셀에 추가)
# visualize_defect_types_samples(data_dir, clientIdentifierDict, label_mapping, min_pixels=100)