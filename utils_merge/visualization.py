...
from utils_merge.dataset_functions import unwrap_client_data
from utils_merge.image_processing import *

def calculate_image_dimensions_from_tiles(num_tiles, tileSize=128):
    '''
    Calculate image height and width from number of tiles.
    Assumes square images (height == width).
    '''
    # Calculate tiles per side (assuming square image)
    tiles_per_side = int(np.sqrt(num_tiles))
    height = width = tiles_per_side * tileSize
    return height, width

def visualize_results_testset(model, imageDictTest, segMaskDictTest, 
                              testClients, clientIdentifierDict, tileSize=128):
    '''
    ...
    for clientID in testClients:
        ...
        for build in clientIdentifierDict[clientID]:
            ...
            if build == '0000216':
                ...
                # For merged_data images: calculate from actual tile count
                # We need to determine how many tiles belong to this image
                # Since images are processed sequentially, we can count tiles
                # But we don't know the count in advance, so we'll use a default
                # For merged_data: images are 640x640 after crop = 5x5 = 25 tiles
                num_tiles_per_image = 25  # 5*5 for 640x640 images
    ...
    for clientID in testClients:
        ...
        for build in clientIdentifierDict[clientID]:
            ...
            if build == '0000216':
                ...
                # For merged_data: 640x640 = 5*128 x 5*128 = 25 tiles
                num_tiles_per_image = 25  # 5*5 for 640x640 images
                curr_idx = prev_idx + num_tiles_per_image
                imageheight, imagewidth = 5*128, 5*128  # 640x640
            ...
            print("Defect IoU = ", np.round(fl_iou_defect,3), " / ", np.round(fl_iou_defect,3))
